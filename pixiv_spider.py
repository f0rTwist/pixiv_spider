        ...
        while True:
            ...
            if len(self.list_url) == 0:
                ...
                if not os.path.exists(
                        str(self.params_rank['mode']) + '-' + (
                                self.date if self.date != '' else time.strftime("%Y%m%d", time.localtime()))):
                    os.mkdir(
                        str(self.params_rank['mode']) + '-' + (self.date if self.date != '' else time.strftime("%Y%m%d",
                                                                                                               time.localtime())))
                ...
                with open(path_, 'wb') as f:
                    ...
                    print(time.strftime("%H:%M:%S", time.localtime()) + ' 图片' + sid + '正在保存...')
                    self.i += 1
...
def main():
    ...
    print('图片保存到' + pixivc.params_rank['mode'] + '-' + (
        str(date) if str(date) != '' else time.strftime("%Y%m%d", time.localtime())))
    for i in range(3):
